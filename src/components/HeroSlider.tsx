  ...
  return <section className="relative h-screen overflow-hidden">
      <div className="embla h-full" ref={emblaRef}>
        <div className="embla__container flex h-full">
          {slides.map((slide, index) => <div key={index} className="embla__slide flex-none w-full h-full relative">
              {/* Parallax Background */}
              <div className="absolute inset-0 w-full h-full will-change-transform transition-transform duration-1000 ease-out" style={{
              ...
              <div className="relative z-10 h-full flex items-center justify-center px-6">
                <div className="text-center max-w-4xl mx-auto">
                  ...
                  <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-12 md:p-16 shadow-2xl">
                    ...
                    <p className="text-xl md:text-2xl text-white/90 mb-12 font-light leading-relaxed animate-fade-in" style={{
                  animationDelay: '0.2s'
                }}>
                  ...
                  animationDelay: '0.4s'
                      ...
                      <Button size="lg" className="bg-white/20 hover:bg-white/30 text-white border border-white/30 backdrop-blur-sm rounded-full px-12 py-4 text-lg font-light transition-all duration-300 hover:scale-105" asChild>
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3">
        {slides.map((_, index) => <button key={index} className={`transition-all duration-300 rounded-full ${index === selectedIndex ? 'w-12 h-3 bg-white' : 'w-3 h-3 bg-white/40 hover:bg-white/60'}`} onClick={() => scrollTo(index)} />)}
      </div>
    </section>;
}